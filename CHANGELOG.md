...
Replaced the old light pill-canvas guide design with the v2 dark-editorial brand
(matches the booking site / weekly planner): hero photo, serif/gold-on-dark,
tap-to-copy door code + WiFi key card, **Message Will** action (prefilled SMS),
content reorganized into clean labeled sections. Each property's `index.html` is
now the v2 design.

- Original v1 design archived per property as `<prop>/v1.html` (content source for
  the generator).
- Generated by `build_v2.py`, which parses `<prop>/v1.html` and writes both
  `index.html` (live) and `v2.html` (mirror) in the brand. Re-run it to apply any
  global change to all six at once.
- The checkout "Text Will that you've left" item opens a prefilled checkout text
  on tap. "Dody wash" -> "Body wash" typo fixed.
- Verified: 0 horizontal overflow, tap targets clean, OG images intact, both
  engines (Chromium + WebKit) at 320/390/430.
